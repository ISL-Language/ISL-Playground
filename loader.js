...
/**
 * Class to load ISL files. That's it.
 */
class ISLFileLoader {
  #loaded = false
  constructor() { }
  ...
  setupInput(id) {
    if (id && id.length > 0) {
      let input = (id instanceof HTMLInputElement) ? id : document.getElementById(id)
      if (input) {
      ...
      const inputElement = this.#fromHTML(`<input type="file" width="0" height="0" style="width: 0px; height: 0px; position: absolute; left: 0px; top: 0px; opacity: 0;" title="Upload .isl File" id="isl-loader-file-upload" accept=".isl">`)
      const labelElement = this.#fromHTML(`<label for="isl-loader-file-upload" id="isl-loader-file-upload-label">Upload <code>.isl</code> File</label>`)
      ...
      thisScript.insertAdjacentHTML("afterend", "<!-- File Input injected by ISL File Loader -->")
      return divElement
    }
  }
  uploadFile(file) {
    ...
    if (extension == "isl") {
      ...
      reader.onload = function(e) {
        ...
        self.#loaded = true
        self.#isl = textByLine
        if (self.#loadCallback) { self.#loadCallback(self.#isl) }
      };
      reader.readAsText(file)
    }
    else {
      throw new Error("Incorrect file extension: expected .isl, got ." + extension)
    }
  }
  /**
